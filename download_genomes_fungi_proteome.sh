...
awk '{FS="\t"} !/^#/ {print $20} ' list.txt  | sed -r 's|(ftp://ftp.ncbi.nlm.nih.gov/genomes/all/.+/)(GCF_.+)|\1\2/\2_protein.faa.gz|'|sed s'/identical//' > downloads.txt

echo "MAKING A LIST OF DOWNLOADS..."
cut -f20 list.txt  | sed -r 's|(ftp://ftp.ncbi.nlm.nih.gov/genomes/all/.+/)(GCF_.+)|\1\2/\2_protein.faa.gz|'|sed s'/identical//' > downloads.txt
